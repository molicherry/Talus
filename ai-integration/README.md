...
- Injected block: a bilingual header — *可用服务 / Available services（调用某个
  ...
  `- <name> — <description> | guide/指南: <excerpt or 无/none>`.
...
plugin (OpenCode / pi / Claude Code / Codex). For Claude Code and Codex it
also **auto-registers the hook** — merging into `~/.claude/settings.json` and
`~/.codex/config.toml` + `~/.codex/hooks.json` (additive merge, `.bak` backup,
idempotent; re-running upgrades the plugin). Manual per-platform steps below.
