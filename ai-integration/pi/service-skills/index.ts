...
// into the conversation via the context event, so the AI always sees what
// services are available and is reminded to fetch the per-service usage guide
// before relaying.
//
...
export default function serviceSkillsExtension(pi) {
  if (process.env.TALUS_API_KEY === undefined) return
  pi.on?.("context", async (event, ctx) => {
    try {
      ...
      return {
        message: {
          customType: "service-skills-directory",
          content: block,
          display: false,
        },
      }
